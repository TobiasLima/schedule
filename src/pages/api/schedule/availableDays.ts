...
export default (request: NextApiRequest, response: NextApiResponse) => {
  ...
  const currentDayHours = availableHoursPerDay.filter(item => item.id === selectedWeekDay);
  const rangesMorning = [];
  const rangesAfternoon = [];
  const now = moment();

  let start = moment(currentDayHours[0]?.start, 'HH:mm');
  if(moment(selectedDate).date() === moment().date() && moment().hour() >= start.hour()) {
  ...
  if(moment(selectedDate).date() === moment().date() && moment().hour() >= start2.hour()) {
